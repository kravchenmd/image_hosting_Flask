...
def move_picture(user_id: int, file_path: Path) -> tuple[str, int]:
    """
    Move, rename picture after uploading.
    Return relative path to the file and size for storing in the DB
    """
    target_folder = Path(BASE_DIR / "src" / "static" / str(user_id))
    ...
    file_name_for_db = f"/static/{user_id}/{file.name}"  # relative to `static` path for the DB
